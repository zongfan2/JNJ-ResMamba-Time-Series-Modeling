...
Full Deep TSO experiments are intended to run on Domino because the local machine does not have the required training data and GPU/runtime stack. See `experiments/deep_tso/README.md` for the full runbook. The H5 path / split / output root all come from the config YAMLs, so the run scripts need no env vars (export `INPUT_H5`/`SPLIT_FILE`/`OUTPUT_ROOT` only to override).
