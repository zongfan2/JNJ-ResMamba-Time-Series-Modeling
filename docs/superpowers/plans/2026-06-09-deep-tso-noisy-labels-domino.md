...
8. **NaN-robustness (added after the first Domino run produced `Loss: nan` from epoch 1).** Raw GENEActive parquet has NaN/Inf in gaps/non-wear; a single non-finite value gives NaN logits → NaN loss → NaN weights after one `optimizer.step()`, freezing the run (acc stuck at the majority class, F1=0), and the resulting NaN `selection_score` saves no checkpoint → end-of-run `FileNotFoundError`. Defense-in-depth fixes: (a) `convert_h5` `np.nan_to_num`s `X_seg` at build (source); (b) `run_model_tso_h5` sanitizes `pad_X` before the forward (covers already-built H5); (c) the optimizer step is skipped on non-finite loss (never poisons weights); (d) `epoch_loss` ignores non-finite; (e) the best-model load tolerates a missing checkpoint. Also: build with a `SCALER_PATH` (unscaled input both destabilizes and is unlike how the prior models were trained).

9. **GCE inf-gradient bug + step guards (found via the Domino runs).** Three layered issues surfaced and were fixed: (i) **the real bug** — in the binary branch of `generalized_cross_entropy_loss`, `prob_true = 1 - prob_pos` was unclamped, so once the model got confident on a negative minute (`prob_pos → 1`), `prob_true → 0` and the GCE gradient `∝ prob_true**(q-1)` blew up to `+inf` → NaN gradients on every batch → training stalled and underfit (~0.80 F1 vs the CE baseline's ~0.90). Fixed by clamping `prob_true` to `[1e-7, 1]` before the power (3-class was already safe via the post-softmax clamp). (ii) `optimizer.step()` is now skipped on a non-finite **gradient** (via `clip_grad_norm_`'s returned norm), not just a non-finite loss — a finite loss with a NaN gradient was poisoning all weights through clip-by-NaN. (iii) `skip_cross_attention` was hardcoded `True` but untested; it's now config-driven and defaults to the tested `False` path. Per-epoch non-finite-loss and non-finite-gradient skip counts are printed so silent stalls are visible. Scaler also corrected to `mbav1_scaler.joblib` (matches Deep Scratch) and configs lowered to `batch_size: 8` (the raw-patch model OOMs a 22GB GPU at 24).
